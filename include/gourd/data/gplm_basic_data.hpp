...
#ifndef _GOURD_GPLM_BASIC_DATA_
#define _GOURD_GPLM_BASIC_DATA_
...
template< typename T >
gourd::gplm_basic_data<T>::gplm_basic_data(
  const std::string xfile,
  const std::vector<std::string>& yfiles,
  const std::string surf_file
) {
  if ( yfiles.empty() ) {
    throw std::domain_error("gplm_basic_data: must provide outcome "
			    "data files");
  }
...
template< typename T >
int gourd::gplm_basic_data<T>::n() const {
  return x_.rows();
};


template< typename T >
int gourd::gplm_basic_data<T>::nloc() const {
  return coord_.size();
};


template< typename T >
int gourd::gplm_basic_data<T>::p() const {
  return xudv_.rank();
};
...
template< typename T >
const typename gourd::gplm_basic_data<T>::mat_type&
gourd::gplm_basic_data<T>::y()
  const {
  return y_;
};
...
template< typename T >
const typename gourd::gplm_basic_data<T>::mat_type&
gourd::gplm_basic_data<T>::xsvd_u()
  const {
...
template< typename T >
const typename gourd::gplm_basic_data<T>::mat_type&
gourd::gplm_basic_data<T>::xsvd_v()
  const {
...
template< typename T >
const typename gourd::gplm_basic_data<T>::vector_type&
gourd::gplm_basic_data<T>::xsvd_d()
  const {
...
template< typename T >
const Eigen::BDCSVD<typename gourd::gplm_basic_data<T>::mat_type>&
gourd::gplm_basic_data<T>::xsvd() const {
  return xudv_;
};


template< typename T >
const std::vector<typename gourd::gplm_basic_data<T>::coord_type>&
gourd::gplm_basic_data<T>::coordinates()
  const {
  return coord_;
};
...
template< typename T >
void gourd::gplm_basic_data<T>::read_data_(
  const std::vector<std::string>& yfiles,
  ::gifti_image* surface
) {
  // #ifdef GOURD_PROFILE_COMPUTATIONS
...
template< typename T >
void gourd::gplm_basic_data<T>::process_images_(
  ::nifti_image* metric,
  ::gifti_image* surface,
  const int i
) {
