...
#include "gifti_io.h"
#include "nifti1.h"
// #include "nifti/nifti2/nifti2_io.h"
...
namespace gourd {
  ...
  public:
    ...
    cifti_gifti_pair( ::nifti_image* nim, ::gifti_image* gim );
...
void gourd::def::extract_coordinates(
  ...
  for ( int i : vertex_indices ) {
    ...
    // if ( i < 10 ) {
    //   std::cout << "{";
    //   for ( size_t j = 0; j < 3; j++ )  std::cout << xyz_buff[j] << ", ";
    //   std::cout << "\b\b} \n";
    // }
...
gourd::cifti_gifti_pair::cifti_gifti_pair(
  ...
  for ( size_t i = 0; i < cim_info_.MatrixIndicesMap.size(); i++ ) {
    // std::cout << cim_info_.MatrixIndicesMap[i].IndicesMapToDataType
    // 	      << " (" << cim_info_.MatrixIndicesMap[i].NumberOfSeriesPoints << ")"
    // 	      << "\n";
    // gourd::display( cim_info_.MatrixIndicesMap[i] );
    // std::cout << "\n";
