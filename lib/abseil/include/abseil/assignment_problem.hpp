...
const int abseil::min_cost_assignment::Unassigned = -1;

double abseil::min_cost_assignment::tol_ = 1e-4;
...
void abseil::min_cost_assignment::tolerance( const double tol ) {
  tol_ = std::abs( tol );
};
...
void abseil::min_cost_assignment::initialize(
  ...
  bool flag;
  m_assigned = 0;
  column_labels.assign(N, Unassigned);
  row_lab_f.assign(N, Unassigned);
  U.assign(N, 0);
  V.assign(N, 0);
  ...
  for (i = 0; i < N; i++) {
	...
	if ( flag ) {
      ...
      // end : while ( row_lab_f[i] != Unassigned  &&  j < N )
      if ( column_labels[j] == Unassigned ) {
	m_assigned++;
	row_lab_f[i] = j;
	column_labels[j] = i;
...
bool abseil::min_cost_assignment::equiv(
  ...
  return std::abs(x - y) <= static_cast<T>(tol_);
};
...
int abseil::min_cost_assignment::min_row_index(
  ...
  for (int i = 1; i < N; i++) {
    ...
    // else if ( Cost.coeffRef(i, col) == Cost.coeffRef(imin, col) ) {
      if ( tiebreak[imin] != Unassigned  &&
	   tiebreak[i] == Unassigned ) {
	imin = i;
      }
    }
  }
...
int abseil::min_cost_assignment::min_col_index(
  ...
  for (int j = 1; j < M; j++) {
    ...
    // else if ( C == Cmin ) {
      if ( tiebreak[jmin] != Unassigned  &&
	   tiebreak[j] == Unassigned ) {
	jmin = j;
	Cmin = C;
      }
    }
